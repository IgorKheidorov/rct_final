  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const pagefind = await (new Function('return import("/pagefind/pagefind.js")')() as Promise<
          Record<string, (...args: unknown[]) => unknown>
        >)
        await (pagefind.init as () => Promise<void>)()
        const res = await (pagefind.search as (q: string) => Promise<{ results: Array<{ data: () => Promise<SearchResult> }> }>)(
          query,
        )
  ...
  return (
    <div className="space-y-[2px]">
      ...
      {results.map((result, i) => (
        ...
        >
          <p className="text-[11px] font-body uppercase tracking-label text-text-muted mb-1">{result.url}</p>
